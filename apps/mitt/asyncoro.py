            ...
            def poll(self):
                ...
                while True:
                    ...
                    events = [(self._fds.get(fid, None), event) \
                              for (fid, event) in list(events.items())]
                    self._lock.release()
                    iocp_notify = False
        ...
        class _AsyncNotifier(object, metaclass=MetaSingleton):
            ...
            def _del_timeout(self, fd):
                if fd._timeout_id:
                    ...
                    for i in range(i, len(self._timeouts)):
...
if not isinstance(getattr(sys.modules[__name__], '_AsyncNotifier', None), MetaSingleton):
    ...
    class _KQueueNotifier(object, metaclass=MetaSingleton):
        """Internal use only.
        """
...
class AsynCoro(object, metaclass=MetaSingleton):
    ...
    def terminate(self, await_non_daemons=False):
        ...
        if not self._terminate:
            if await_non_daemons and len(self._coros) > self._daemons:
                ...
                self._complete.wait()
            if self._location:
                for x in range(len(self._netreq_q)):
                    time.sleep(0.1)
                ...
                def _terminate(self, req, coro=None):
                    ...
                    sock.close()
                for (addr, port), auth in list(self._peers.items()):
    ...
    def send_file(self, location, file, dest_path=None, overwrite=False, timeout=None):
        ...
        transferred) and 2 if a file with same name is at the
        destination, but 'overwrite' is False. If return value
        is 0, the sender may want to delete file with 'del_file'
        later.
        """
        try:
        ...
        if not ((stat.S_IMODE(stat_buf.st_mode) & stat.S_IREAD) and stat.S_ISREG(stat_buf.st_mode)):
            raise StopIteration(-1)
        kwargs = {'file':os.path.basename(file), 'stat_buf':stat_buf, 'overwrite':overwrite == True}
    ...
    def _tcp_task(self, conn, addr, coro=None):
        ...
        if req.request == 'send':
            ...
            try:
                ...
                pending_reqs = list(self._requests.values())
